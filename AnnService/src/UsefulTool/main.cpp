...
//#include "inc/SSDServing/Utils.h" this statement is the root cause, why?
...
public:
    ToolOptions():Helper::ReaderOptions(VectorValueType::Float, 0, VectorFileType::TXT, "|", 32)
    {
        ...
        AddOptionalOption(buildVector, "-BuildVector", "--BuildVector", "1");
        AddOptionalOption(m_baseVectorPath, "-BaseVectorPath", "--BaseVectorPath", "1");
        AddOptionalOption(m_queryVectorPath, "-QueryVectorPath", "--QueryVectorPath", "1");
        AddOptionalOption(m_baseVectorSplitPath, "-BaseVectorSplitPath", "--BaseVectorSplitPath", "1");
        AddOptionalOption(ride, "-ride", "--ride", "1");
        AddOptionalOption(m_configurePath, "-ConfigurePath", "--ConfigurePath", "1");
        AddOptionalOption(newQueryDataSetFileName, "-NewQueryDataSetFileName", "--NewQueryDataSetFileName", "1");
    }
    ...
    int ride;
    bool buildVector;
    std::string m_baseVectorPath;
    std::string m_queryVectorPath;
    std::string m_baseVectorSplitPath;
    std::string m_configurePath;
    std::string newQueryDataSetFileName;
...
void ConvertTruth(ToolOptions& p_opts)
{
    ...
    std::string truthFileAfter = truthFile + ".after" /*+ std::to_string(p_opts.batch)*/;
...
void CallRecall(ToolOptions& p_opts)      
{
    ...
    //LOG(Helper::LogLevel::LL_Info, "Start Loading Search Results\n");
    ...
    //LOG(Helper::LogLevel::LL_Info, "vectorSet num:%d\n", vectorSet->Count());
    //LOG(Helper::LogLevel::LL_Info, "querySet num:%d\n", querySet->Count());
    //LOG(Helper::LogLevel::LL_Info, "truthfile: %s\n", truthfile.c_str());
    //LOG(Helper::LogLevel::LL_Info, "searchResultfile: %s\n", p_opts.m_searchResult.c_str());
    //LOG(Helper::LogLevel::LL_Info, "ids size:%d, dists size:%d\n", ids.size(), dists.size());
    ...
    for (SizeType i = 0; i < NumQuerys; i++)
    {
        ...
        {
            //LOG(Helper::LogLevel::LL_Info, "for vector id %d in truth for query vector %d :\n", id, i);
            ...
            for (int j = 0; j < K; j++)
            {
                ...
                if (vectorSet != nullptr) {
                    ...
                    //LOG(Helper::LogLevel::LL_Info, "the recorded distance between vector id %d in truth and query vector $d is: %f, and the actual dis is %f\n", id, i, dist, truthDist);
...
template <typename ValueType>
void BuildVector(ToolOptions& p_opts)
{   
    std::vector<SizeType> current_list;
    SizeType batch = p_opts.batch;
    std::string last_path = "/home/laiyuhui/datasets/argoverse2/motion_forecast/embeddings/test/last_base_vector_id.txt";

    //list contains vectors' id
    LOG(Helper::LogLevel::LL_Info, "Begin Generating Trace\n");
    std::shared_ptr<VectorSet> baseVectorSet;
    std::shared_ptr<VectorSet> queryVectorSet;
    std::vector<SizeType> baseVectorSplitList;


    LOG(Helper::LogLevel::LL_Info, "Loading Base Split Range Set\n");
    if (!p_opts.m_baseVectorSplitPath.empty() && fileexists(p_opts.m_baseVectorSplitPath.c_str())) {
        // p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter : -v��-d��-f��-dl��
        // -v��-f��-dl are assigned when initializing p_opts
        std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(vectorOptions);

        if (ErrorCode::Success == vectorReader->LoadFile(p_opts.m_baseVectorSplitPath))
        {
            baseVectorSplitList = vectorReader->GetVectorSplitSet();
            
            LOG(Helper::LogLevel::LL_Info, "\nLoad VectorSplitSet(%d).\n", baseVectorSplitList.size());
        }
    }

    LOG(Helper::LogLevel::LL_Info, "\nVectorSplitSet(%d) = %d.\n", batch, baseVectorSplitList.at(batch));

    std::ifstream inFile;
    inFile.open(last_path, std::ios::in);
    ...
    if(!inFile) {
      ...
      {
          last_end = std::stoi(str);
          //LOG(Helper::LogLevel::LL_Info, "\nlast_end = %d.\n", last_end);
      }
    }
    inFile.close();

    SizeType end = baseVectorSplitList.at(batch);
    //std::srand(std::time(0));
    //SizeType last_end = batch == 0 ? -1 : baseVectorSplitList.at(batch - p_opts.ride);
    
    //SizeType current_inserted = /*((int)std::rand() % (end - last_end))*/((end - last_end) * ((float)p_opts.ride / (float)(p_opts.ride+1000))) + last_end + 1;
    
    /*std::ofstream oFile;
    oFile.open(last_path, std::ios::out);
    oFile << current_inserted;
    oFile.close();*/
    
    //LOG(Helper::LogLevel::LL_Info, "((end - last_end) * (p_opts.ride / (p_opts.ride+1000))) = %d, (end - last_end) = %d, (p_opts.ride / (p_opts.ride+1000)) = %f \n", end-last_end, end-last_end, (float)p_opts.ride / (float)(p_opts.ride+1000));
    //LOG(Helper::LogLevel::LL_Info, "end=%d, last_end = %d, current_inserted=%d, ride=%d\n", end, last_end, current_inserted, p_opts.ride);

    LOG(Helper::LogLevel::LL_Info, "Loading Base Vector Set\n");
    if (!p_opts.m_baseVectorPath.empty() && fileexists(p_opts.m_baseVectorPath.c_str())) {
        // p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter : -v��-d��-f��-dl��
        // -v��-f��-dl are assigned when initializing p_opts
        std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(vectorOptions);
        if (ErrorCode::Success == vectorReader->LoadFile(p_opts.m_baseVectorPath))
        {
            baseVectorSet = vectorReader->GetVectorSet(0, /*current_inserted*/end + 1);
            if (p_opts.m_distCalcMethod == DistCalcMethod::Cosine) 
                baseVectorSet->Normalize(4);

            LOG(Helper::LogLevel::LL_Info, "\nLoad VectorSet(%d, %d, %d).\n", end, baseVectorSet->Count(), baseVectorSet->Dimension());
        }
    }

    LOG(Helper::LogLevel::LL_Info, "Loading Query Vector Set\n");
    if (!p_opts.m_queryVectorPath.empty() && fileexists(p_opts.m_queryVectorPath.c_str())) {
        // p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter : -v��-d��-f��-dl��
        // -v��-f��-dl are assigned when initializing p_opts
        std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(p_opts.m_inputValueType, p_opts.m_dimension, p_opts.m_inputFileType, p_opts.m_vectorDelimiter));
        auto vectorReader = Helper::VectorSetReader::CreateInstance(vectorOptions);
        if (ErrorCode::Success == vectorReader->LoadFile(p_opts.m_queryVectorPath))
        {
            queryVectorSet = vectorReader->GetVectorSet(0, batch + 1);
            if (p_opts.m_distCalcMethod == DistCalcMethod::Cosine)
                queryVectorSet->Normalize(4);

            LOG(Helper::LogLevel::LL_Info, "\nLoad VectorSet(%d,%d).\n", queryVectorSet->Count(), queryVectorSet->Dimension());
        }
    }

    LOG(Helper::LogLevel::LL_Info, "Initialize list\n");
    if (p_opts.batch == 0) {
        current_list.resize(baseVectorSet->Count());
        for (int i = 0; i < baseVectorSet->Count(); i++)
            current_list[i] = i;
    }
    else {
        LoadUpdateMapping(p_opts.currentListFileName + std::to_string(p_opts.batch - p_opts.ride), current_list);
        
        SizeType current_list_length = current_list.size();

        for (SizeType st = current_list_length; st <= end; st++) {
            current_list.push_back(st);
        }
        
        LOG(Helper::LogLevel::LL_Info, "current_list length %d to %d\n", current_list_length, current_list.size());
    }
    
    std::sort(current_list.begin(), current_list.end());
    SaveUpdateMapping(p_opts.currentListFileName + std::to_string(p_opts.batch), current_list, baseVectorSet->Count());


    //generate current vector dataset
    LOG(Helper::LogLevel::LL_Info, "Generate new dataset\n");
    COMMON::Dataset<ValueType> newSample(0, p_opts.m_dimension, 1024 * 1024, MaxSize);//p_opts.m_dimension : -d
    COMMON::Dataset<ValueType> newQuerySample(0, p_opts.m_dimension, 1024 * 1024, MaxSize);
    
    for (int i = 0; i <= end; i++) {
        newSample.AddBatch(1, (ValueType*)(baseVectorSet->GetVector(current_list[i])));
    }
    
    LOG(Helper::LogLevel::LL_Info, "Saving current new dataset\n");
    newSample.Save(p_opts.newDataSetFileName + std::to_string(p_opts.batch));

    for (int i = 0; i < batch + 1; i++) {
        newQuerySample.AddBatch(1, (ValueType*)(queryVectorSet->GetVector(i)));
    }
    
    LOG(Helper::LogLevel::LL_Info, "Saving current new query dataset\n");
    newQuerySample.Save(p_opts.newQueryDataSetFileName + std::to_string(p_opts.batch));

    /*
    //search current results
    std::map<std::string, std::map<std::string, std::string>> my_map;
    LOG(Helper::LogLevel::LL_Info, "configurePath = %s.\n", p_opts.m_configurePath.c_str());
    auto ret = SSDServing::BootProgram(false, &my_map, p_opts.m_configurePath.c_str());*/
}
...
int main(int argc, char* argv[]) {
    ...
    if (options.buildVector) {
      switch (options.m_inputValueType) {
      case SPTAG::VectorValueType::Float:
          BuildVector<float>(options);
          //CallRecall<float>(options);
          break;
      case SPTAG::VectorValueType::Int16:
          BuildVector<std::int16_t>(options);
          //CallRecall<std::int16_t>(options);
          break;
      case SPTAG::VectorValueType::Int8:
          BuildVector<std::int8_t>(options);
          //CallRecall<std::int8_t>(options);
          break;
      case SPTAG::VectorValueType::UInt8:
          BuildVector<std::uint8_t>(options);
          //CallRecall<std::uint8_t>(options);
          break;
      default:
          LOG(Helper::LogLevel::LL_Error, "Error data type!\n");
      ...
      //search current results
      std::map<std::string, std::map<std::string, std::string>> my_map;
      LOG(Helper::LogLevel::LL_Info, "configurePath = %s.\n", options.m_configurePath.c_str());
      auto ret = SSDServing::BootProgram(false, &my_map, options.m_configurePath.c_str());
    }
