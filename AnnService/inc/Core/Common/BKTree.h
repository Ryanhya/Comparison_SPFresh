                    ...
                    while (!ss.empty()) {
                        ...
                        if (item.last - item.first <= m_iBKTLeafSize) {
                            ...
                            if (numClusters <= 1) {
                                ...
                                std::cout<<data.C()<<","<<sizeof(T)<<std::endl;
                                std::cout<<"no."<<0<<"vector: [";
                                for(int zz = 1; zz < data.C(); zz++){
                                    T single_d= *addr;
                                    addr += zz*sizeof(T);
                                    std::cout<<single_d;
                                ...
                                /*for (SizeType z = item.first; z < item.last; z++)
                                {                              
                                    const T* addr = data[localindices[z]];

                                    std::cout<<"no."<<z<<"vector: [";
                                    for(int zz = 1; zz < data.C(); zz++){
                                        T single_d= *addr;
                                        addr += zz*sizeof(T);
                                        std::cout<<single_d;
