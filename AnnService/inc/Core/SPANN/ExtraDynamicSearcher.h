...
namespace SPTAG::SPANN {
    ...
    public:
        ...
        ErrorCode Split(VectorIndex* p_index, const SizeType headID, bool reassign = false, bool preReassign = false)
        {
            ...
            {
                std::unique_lock<std::shared_timed_mutex> lock(m_rwLocks[headID]/*, std::defer_lock*/);//lock posting headID

                std::string postingList;
                auto splitGetBegin = std::chrono::high_resolution_clock::now();
                //the modification mark
                //lock.lock();
                ...
                if(dis1 < Epsilon || dis2 < Epsilon) {
                    if(std::min(args.counts[0],args.counts[1]) / static_cast<double>(localIndices.size()) <= 0.1){
                        //LOG(Helper::LogLevel::LL_Info, "Same head\n");

                        b = args.counts[0] < args.counts[1] ? 0 : 1;
                        int c = args.counts[0] < args.counts[1] ? 1 : 0;
                ...
                for (int k = 0; k < 2; k++) {
                    ...
                    if (!theSameHead && p_index->ComputeDistance(args.centers + k * args._D, p_index->GetSample(headID)) < Epsilon) {                        
                        ...
                        //std::unique_lock<std::shared_timed_mutex> lock(m_rwLocks[begin]);
        ...
        ErrorCode ConcurrentSplit(VectorIndex* p_index, const SizeType headID, bool reassign = false, bool preReassign = false){
            ...
            double elapsedMSeconds;
            {
                ...
                int index = 0;
                uint8_t* vectorId = postingP;
                for (int j = 0; j < postVectorNum; j++, vectorId += m_vectorInfoSize)//the loop is to get the all vectors that not be deleted or updated(holds a lower version value)
                {
                ...
                for (int k = 0; k < 2; k++) {
                    ...
                    if (!theSameHead && p_index->ComputeDistance(args.centers + k * args._D, p_index->GetSample(headID)) < Epsilon) {
                        ...
                        newHeadsID.push_back(begin);
                        auto splitPutBegin = std::chrono::high_resolution_clock::now();
                        if (!preReassign && db->Put(newHeadVID, newPostingLists[k]) != ErrorCode::Success) {//write new posting data to ssd
                            LOG(Helper::LogLevel::LL_Info, "Fail to add new postings\n");
                            exit(0);
                        }
                        auto splitPutEnd = std::chrono::high_resolution_clock::now();
        ...
        checkDeleted:
            ...
            double appendIOSeconds = 0;
            {
                ...
                //the modification mark
                //std::lock_guard<std::mutex> tmplock(m_dataAddLock);
                m_postingSizes.IncSize(headID, filtered_num/*appendNum*/);//CAS operation that modifies the postingSize
            }
...
//                     }
                    ...
                    for (SizeType j = start; j < end; j++) {
                        ...
                        if (headVectorIDS.count(j) == 0) {//if vector j is not the centroid vector, calculate the postingSize and replicate the vector
    ...
    //         }
        ...
        void SavePostingSizesAndVersionMap(){
    ...
    // #pragma omp parallel for num_threads(10)
        ...
        void GetDBStats() override { 
            ...
            LOG(Helper::LogLevel::LL_Info, "current posting num in postingSizes: %d\n", m_postingSizes.GetPostingNum());
        }
        ...
        void ShowPostingDistribution(int num, bool needPrint){
            ...
            if(needPrint){
                std::string file_name = "/home/lyh/PostingDistributions/SPFresh_baseline/PostingDistribution"+std::to_string(num)+".csv";
        ...
        void calculatePostingSizeMSE(){
            ...
            std::string file_name = "/home/lyh/PostingDistributions/SPFresh_baseline/PostingDistribution.csv";
