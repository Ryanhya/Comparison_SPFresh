...
namespace SPTAG {
	namespace SSDServing {
        namespace SPFresh {
                ...
                auto func = [&]()
                {
                    ...
                    while (true)
                    {
                        while (pause_flag) {
                            ...
                            cv.wait(lock, [] { return !pause_flag; });
                        }
            ...
            template <typename ValueType>
            void ConcurrentInsertVectors(SPANN::Index<ValueType>* p_index, 
                int insertThreads, 
                std::shared_ptr<SPTAG::VectorSet> vectorSet, 
                int curCount, 
                int step,
            ...
            template <typename ValueType>
            void ConcurrentUpdate(SPANN::Index<ValueType>* p_index)
            {
                SPANN::Options& p_opts = *(p_index->GetOptions());
                int step = p_opts.m_step;//the default value is 0
                if (step == 0)
                {
                    LOG(Helper::LogLevel::LL_Error, "Incremental Test Error, Need to set step.\n");
                    exit(1);
                ...
                if (!p_opts.m_baseVectorSplitPath.empty() && fileexists(p_opts.m_baseVectorSplitPath.c_str())) {
                    ...
                    std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(p_opts.m_valueType, p_opts.m_dim, p_opts.m_queryType, p_opts.m_queryDelimiter));
                    auto vectorReader = Helper::VectorSetReader::CreateInstance(vectorOptions);
                ...
                if (ErrorCode::Success != vectorReader->LoadFile(p_opts.m_fullVectorPath))
                {
                    LOG(Helper::LogLevel::LL_Error, "Failed to read base vector file.\n");
                    exit(1);
                }
                ...
                if (!p_opts.m_fullVectorPath.empty() && fileexists(p_opts.m_fullVectorPath.c_str())) {//BuildSSDIndex -> FullVectorPath
                    ...
                    for (int m = p_opts.m_startNum; m < baseVectorSplitList.size(); m = m + step) {
                        ...
                        std::shared_ptr<VectorSet> subBaseSet;
                        std::shared_ptr<VectorSet> subQuerySet;
                    
                        subBaseSet = vectorReader->GetVectorSet(0, split_index + 1);
                        subQuerySet = queryReader->GetVectorSet(0, m + 1);
            
                        int curVectorCount ;//version map num
                        if(m == p_opts.m_startNum){
                            curVectorCount = p_index->GetNumSamples();
                        }
                        else{
                            curVectorCount = baseVectorSplitList.at(m - step)+1;
                        }

                        int insertCount = subBaseSet->Count() - curVectorCount;//current vectors that need to be inserted num
                        //std::shared_ptr<VectorSet> newInsertVectorSet = vectorReader->GetVectorSet(split_index - insertCount + 2, split_index + 1);
                        ...
                        std::future<void> insert_future =
                            std::async(std::launch::async, ConcurrentInsertVectors<ValueType>, p_index,
                                insertThreads, subBaseSet, curVectorCount, insertCount, std::ref(p_opts));//here: vectorSet should contain the old vectors and all the new current vectors.
                        ...
                        std::string truthFileName = p_opts.m_truthFilePrefix +"_"+ std::to_string(m);
                        ...
                        if(p_opts.m_streamingUpdate){
                            do {
                                ...
                                if (insert_status == std::future_status::timeout && notSearch) {
                                    ...
                                    LOG(Helper::LogLevel::LL_Info, "Current Vector num: %d, target vector num: %d, all new inserted vector num: %d, the percent is %.2lf \n", p_index->GetNumSamples(), split_index+1, insertCount, (p_index->GetNumSamples()-split_index-1+insertCount)/static_cast<double>(insertCount));
                            ...
                            int time_count = 0;
                            int final_count = p_opts.m_searchWaitMilliTime / p_opts.m_smallestTimeUnit;
                            do {
                        ...
                        LOG(Helper::LogLevel::LL_Info, "Total Vector Num: %d. Target Vector Num: %d\n", curVectorCount, split_index+1);
                        
                        //p_index->GetIndexStat(finishedInsert, true, true);//print the index states
                        ...
                        //p_index->StopMerge();
                        //warning: the comment may be removed
                        /*if (p_opts.m_maxInternalResultNum != -1)
                        {
                            for (int iterInternalResultNum = p_opts.m_minInternalResultNum; iterInternalResultNum <= p_opts.m_maxInternalResultNum; iterInternalResultNum += p_opts.m_stepInternalResultNum)
                            {
                                StableSearch(p_index, numThreads, subQuerySet, subBaseSet, searchTimes, p_opts.m_queryCountLimit, iterInternalResultNum, truthFileName, p_opts, sw.getElapsedSec());
                            }
                        }
                        else
                        {
                            StableSearch(p_index, numThreads, subQuerySet, subBaseSet, searchTimes, p_opts.m_queryCountLimit, internalResultNum, truthFileName, p_opts, sw.getElapsedSec());
                        }*/
                        //p_index->OpenMerge();

                        /*if(p_index->GetMemoryIndex()->NeedRefine()){
                            std::shared_ptr<std::vector<std::string>> indexfiles = p_index->GetMemoryIndex()->GetIndexFiles();
                            std::string folderPath(p_opts.m_indexDirectory + FolderSep + p_opts.m_headIndexFolder);
                            ...
                            for (std::string& f : *indexfiles) {
                                ...
                                if (ptr == nullptr || !ptr->Initialize(newfile.c_str(), std::ios::binary | std::ios::out)) return;
                                handles.push_back(std::move(ptr));
                            }

                            p_index->GetMemoryIndex()->RefineIndex(handles, nullptr);
                            std::shared_ptr<VectorIndex> new_pIndex;
                            switch (p_opts.m_valueType)
                            {
#define DefineVectorValueType(Name, Type) \
    case VectorValueType::Name: \
        new_pIndex =  std::shared_ptr<VectorIndex>(new BKT::Index<Type>); \
...
#undef DefineVectorValueType   
                            ...
                            new_pIndex->LoadIndexData(handles);
                            p_index->SetMemoryIndex(new_pIndex);

                        }*/
            
                        if (m == p_opts.m_ternimateNum){
                            ...
                            LOG(Helper::LogLevel::LL_Info, "All background threads finish in %.2lf seconds\n", duration);
                            ShowMemoryStatus(subBaseSet, sw.getElapsedSec());
                            StableSearch(
                                p_index,
                                numThreads,
                    ...
                    if (p_index->GetMemoryIndex()->SaveIndex(p_opts.m_indexDirectory + FolderSep + p_opts.m_headIndexFolder) != ErrorCode::Success) {
                        LOG(Helper::LogLevel::LL_Error, "Failed to save head index.\n");
                    }

                    //store some key data members of the object ExtraSearcher
                    //p_index->GetDiskIndex()->SavePostingSizesAndVersionMap();
                }
            ...
            int UpdateTest(const char* storePath) {
                ...
                //LOG(Helper::LogLevel::LL_Info, "storePath: %s \n", storePath);
            ...
            #define DefineVectorValueType(Name, Type) \
                if (opts->m_valueType == VectorValueType::Name) { \
                    if (opts->m_steadyState) SteadyStateSPFresh((SPANN::Index<Type>*)(index.get())); \
                        ...
                        ConcurrentUpdate((SPANN::Index<Type>*)(index.get()));\
                    }\
                } \
